function Blog() {
    ...
    return (
        /* FIX 3: mt-20 trên mobile, giữ mt-[11%] trên desktop */
        <main className="home mx-auto w-full md:w-[80%] px-4 mt-20 md:mt-[11%]">
            <div className="bg-white rounded-lg p-3 mb-3">
                {/*
                    FIX 1: Đổi flex-col md:flex-row → flex-col-reverse md:flex-row
                    Trên mobile: content chính hiện trước, sidebar xuống sau
                    Trên desktop: giữ nguyên sidebar bên trái, content bên phải
                */}
                <div className="flex flex-col-reverse md:flex-row">

                    {/* SIDEBAR */}
                    <aside className="w-full md:w-1/4 p-4 bg-gray-50 md:mr-5 mt-4 md:mt-2 rounded-lg border border-gray-100">
                            ...
                            {recentNews.map((item, i) => (
                                <li key={i}>
                                    <a href={item.href} target="_blank" rel="noreferrer"
                                        /* FIX 4: hover:text-orange-500 thay vì amber-200 gần như vô hình */
                            ...
                            {allNews.map((item, i) => (
                                <div key={i} className="flex items-start gap-4 pb-6 border-b border-gray-100 last:border-0">
                                    <a href={item.href} target="_blank" rel="noreferrer" className="flex-shrink-0">
                                        {/*
                                            FIX 2: ảnh nhỏ hơn trên mobile — w-24 h-20 (96×80px)
                                            Desktop giữ nguyên w-48 h-32
                                        */}
                                        <img
                                            src={item.img}
                                            alt={item.title}
                                            className="w-24 h-20 md:w-48 md:h-32 object-cover rounded-lg hover:opacity-90 transition-opacity"
                                        />
                                    </a>
                                    <div className="flex-1 min-w-0">
                                        <a href={item.href} target="_blank" rel="noreferrer">
                                            <h3 className="text-sm md:text-base font-bold line-clamp-2 hover:text-orange-500 transition-colors mb-2">
                                                {item.title}
                                            </h3>
                                        </a>
